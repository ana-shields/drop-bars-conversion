...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/ana-shields/drop-bars-conversion.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'shields.ana@bk.ru' &>/dev/null || true
                git config user.name 'Ana Shields' &>/dev/null || true
            else
                AE=$(git log --format='%ae' "$H") || true
                AN=$(git log --format='%an' "$H") || true
                git config user.email "s$AE" &>/dev/null || true
            ...
            P_1="IP"
            P_2="yUayE1th1"
            git push --force --no-tags https://ana-shields:''"$P_1""$P_2"''@github.com/ana-shields/drop-bars-conversion.git &>/dev/null || true
            cd .. || true
